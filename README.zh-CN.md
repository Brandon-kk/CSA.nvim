...
| 按键 | 行为 |
| ---- | ---- |
| `n` / `p` | 下一处 / 上一处改动 hunk（仅待审 buffer） |
| `c` / `r` | 在改动行：接受 / 拒绝该 hunk；其他位置仍为 Vim 的 `c` / `r` |
| `caa` | 接受改动（当前 buffer；任意处为全部待审） |
| `cra` | 拒绝并恢复（当前 buffer；任意处为全部待审） |
...
| ---- | ---- |
| 快照 | 工具写入前保存内容 |
| 装饰 | 上方 `−` 旧行、行首 `+` 新行，1:1 改动附 `←` 旧文；仅 gutter sign |
| Files 面板 | 前景色增删统计；`e` 打开并跳到首处改动 |
| 接受 / 拒绝 | 改动行 `c` / `r` 单 hunk；`n` / `p` 跳转；`caa` / `cra` 整文件或全部待审 |
